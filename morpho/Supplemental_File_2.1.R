######################################
# NMDS of mikromelanos sp n vs zeteki#
#                                    #
# Cody Raul Cardenas Feb 2021        #
######################################
library(tidyverse)
library(readr)
library(vegan)
...
#import data
d <- read_csv("Supplemental_File1.1.csv") %>% drop_na() %>% filter(!duplicated(bc_num))
dw <- d %>% filter(caste != "gyne", caste != "male") #171 workers
dw_BR <- dw %>% filter(species !="mikromelanos")
dw_LB <- dw %>% filter(species !="zeteki")
dg <- d %>% filter(caste != "worker", caste != "male") #53 gynes
dg_BR <- dg %>% filter(species !="mikromelanos")
dg_LB <- dg %>% filter(species !="zeteki")
dm <- d %>% filter(caste != "gyne", caste != "worker") #43 males
dm_BR <- dm %>% filter(species !="mikromelanos")
...
write.table(wLBmorph_tab, file="morph_Mmik_workers.csv", sep=",", row.names=T, col.names = T)
write.table(gBRmorph_tab, file="morph_Mzet_gynes.csv", sep=",", row.names=T, col.names = T)
write.table(gLBmorph_tab, file="morph_Mmik_gynes.csv", sep=",", row.names=T, col.names = T)
write.table(mBRmorph_tab, file="morph_Mzet_males.csv", sep=",", row.names=T, col.names = T)
write.table(mLBmorph_tab, file="morph_Mmik_males.csv", sep=",", row.names=T, col.names = T)
...
welch_tab <- purrr::map_df(list(welch_wHW, welch_wML, welch_wGL, welch_wTL,
  ...
  mutate("mikromelanos mean"=estimate1,"zeteki mean"=estimate2,"t-statistic"=statistic,DF=parameter,
         "confidence intverval low" = conf.low, "confidence interval high" = conf.high) %>%
  select("mikromelanos mean","zeteki mean", "t-statistic",DF,p.value) %>%
  as.matrix()
...
############
set.seed(666)
#calculate disimilarity; see vegan documentation about non-community data
#workers
...
# shepards plots
stressplot(dw.mds1.3,dw.dis, main="Worker NMDS stress plot k=2")
...
wtype_para <- head(as.data.frame(scores(dw.mds1.3)),14)
row.names(wtype_para) <- head(dw$bc_num,14)
wtype_para$spp<-head(dw$species,14)
#get all other points and names
wpoints <- as.data.frame(scores(dw.mds1.3))
row.names(wpoints) <- dw$bc_num
wpoints$spp <-dw$species
#plot using ggord & ggplot functions
worker_NMDS <- ggord(dw.mds1.3, axes=c("2","3"), grp_in=dw$species,
